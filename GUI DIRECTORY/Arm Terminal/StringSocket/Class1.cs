...
namespace CustomNetworking {
    ...
    public class StringSocket : IDisposable {
        ...
        public StringSocket(Socket s, Encoding e) {
            socket = s;
            if (socket.Connected) {
                Console.WriteLine("Socket passed to stringSocket is properly connected when passed");
            }
            else {
                Console.WriteLine("Socket passed to stringSocket is !!!NOT!!! properly connected when passed");
            }
            encoding = e;
        ...
        public void close() {
            Console.WriteLine("Close method called on stringSocket");
            try {
                socket.Shutdown(SocketShutdown.Receive);
                socket.Disconnect(false);
            }
            catch {
                Console.WriteLine("Close method called on stringSocket's underlying socket, meaning it failed to shutdown and/or disconnect gracefully");
                socket.Close();
            }
        }
        ...
        private void ProcessSendQueue() {
            while (sendRequests.Count > 0) {
                ...
                try {
                    ...
                    Console.WriteLine("Exception in stringSocket in ProcessSendQueue");
        ...
        private void BytesSent(IAsyncResult ar) {
            try {
                ...
                Console.WriteLine("Exception in BytesSent in StringSocket after socket.endSend");
                SendRequest req = sendRequests.Dequeue();
                ThreadPool.QueueUserWorkItem(x => req.Callback(e, req.Payload));
                ProcessSendQueue();
                return;
            ...
            // If all the bytes weren't sent, send the rest.
                ...
                try {
                    ...
                    Console.WriteLine("Exception in BytesSent in StringSocket after socket.beginSend");
                    SendRequest req = sendRequests.Dequeue();
                    ThreadPool.QueueUserWorkItem(x => req.Callback(e, req.Payload));
                    ProcessSendQueue();
                }
            ...
            lock (receiveRequests) {
                ...
                while (receiveRequests.Count > 0) {
                    try {
                        ...
                        Console.WriteLine("Exception in stringSocket in ProcessReceiveQueue");
                        ReceiveRequest req = receiveRequests.Dequeue();
        ...
        private void BytesReceived(IAsyncResult ar) {
            ...
            try {
                ...
                Console.WriteLine("Exception in BytesReceived in StringSocket after EndReceive: " + e.Message);
                ReceiveRequest req = receiveRequests.Dequeue();
                ThreadPool.QueueUserWorkItem(x => req.Callback(null, e, req.Payload));
                ProcessReceiveQueue();
                incompleteLine = "";
        ...
        public void Dispose() {
            Console.WriteLine("string socket being disposed");
            close();
        }
    }
}